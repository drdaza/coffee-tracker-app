...
export default function BrowseScreen() {
  ...
  const loadCoffees = useCallback(async () => {
    try {
      await fetchCoffees(queryParams);
    } catch (error) {
      console.error("Failed to load coffees:", error);
    }
  }, [fetchCoffees, queryParams]);
  ...
  const onEndReached = useCallback(() => {
    if (!isLoadingMore && coffeesMeta?.hasNextPage) {
      loadMoreCoffees(queryParams).catch((error) => {
        console.error("Failed to load more coffees:", error);
      });
    }
  }, [isLoadingMore, coffeesMeta?.hasNextPage, loadMoreCoffees, queryParams]);

  const isEmpty = !isLoading && coffees.length === 0;
  const emptyTitle = hasActiveFilters
    ? t("search.noResults")
    : t("browse.noCoffeesYet");
  const emptyMessage = hasActiveFilters
    ? t("search.noResultsMessage")
    : t("browse.noCoffeesMessage");
  ...
  return (
    <ScreenLayout
      ...
      loading={isLoading && coffees.length === 0 && !hasActiveFilters}
      isEmpty={isEmpty}
